...
export default function RecommendedBlogCards(props: any) {
  ...
  return (
    <Link href={`/blog/${blogsList.id}?id=${blogsList.id}`}>
      <Card
        variant="outlined"
        style={{
          marginLeft: "1%",
          ...
          >
            <Grid item xs={6}>
              <Image
                src={
                  blogsList
                    ? imgUrl + blogsList["thumbnail"]?.replace(" ", "%20")
                    : "/24c2337e-6b18-4702-be69-6f5a968e95ed-3.png"
                }
                alt="banner"
                height={135}
                width={235}
            ...
            <Grid item xs={6}>
              <Stack direction="column" spacing={2}>
                ...
                >
                  {blogsList
                    ? moment(blogsList.created_at, "", true).format("Do MMMM")
                    : "12th September"}
                </Typography>
              </Stack>
            </Grid>
